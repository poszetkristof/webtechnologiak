...
### 3.2 Karakter- és entitáshivatkozások

A forrás a hosszú gondolatjelet `--`-ként írta, tipográfiailag helyesen ez egy **em dash**: `—`.
Beírható **karakterhivatkozással**:
...
### 3.4 Feldolgozási utasítás (processing instruction)
...
## 6. Hogyan épül fel egy CSS-stíluslap?
...
### 6.2 Kaszkád, specifikusság, öröklődés
...
## 7. A doboz modell, a display tulajdonság

### 7.1 `display` – miért lesz olvasható a darab?
...
| Függőleges `margin` / `padding` | **működik**                                         | a `padding` látszik, de **nem tolja el a sorokat** |
| Példa HTML-ben                  | `<p>`, `<div>`, `<h1>`                              | `<span>`, `<a>`, `<em>`, `<strong>`                |

### 7.2 A doboz-modell (box model)
